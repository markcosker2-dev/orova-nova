...
from Core_Engine.notifications import alert_safety_trigger
...
class HAWK:
    ...
    async def monitor_roas_safety(self, performance_data: Dict[str, Any]) -> Optional[str]:
        """
        Monitor Meta Ads ROAS and trigger safety measures if below 2.0 or if spend > $500 with 0 leads.
        ...
        # Zero-lead kill switch: pause if spend > $500 with 0 leads, regardless of ROAS
        if spend > 500 and leads == 0:
            alert_message = f"""
            🚨 ZERO LEAD KILL SWITCH ACTIVATED 🚨

            Campaign: {performance_data.get('campaign', 'Unknown')}
            Spend: ${spend}
            Leads: {leads}

            ACTION REQUIRED: Campaign paused due to zero leads despite high spend.
            """

            logger.critical(f"Zero lead kill switch triggered: spend ${spend}, leads {leads}")
            alert_safety_trigger("Zero Lead Kill Switch", f"Spend ${spend}, Leads {leads}")
        ...
        if roas < 2.0:
            ...
            alert_safety_trigger("ROAS Safety", f"ROAS {roas} < 2.0, Spend ${spend}")
            # TODO: Integrate with webhook for phone alert
            return alert_message
