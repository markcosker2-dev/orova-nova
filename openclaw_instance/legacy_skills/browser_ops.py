...
async def google_search_scrape_async(query: str, limit: int = 5) -> List[Dict[str, str]]:
    """
    Perform a direct Google Search using Playwright (Scraper).
    Indestructible fallback when APIs fail.
    """
    results = []
    try:
        from playwright.async_api import async_playwright
    ...
    try:
        ...
        # Go to Google
        # Use explicit wait and user-like behavior
        encoded_query = quote_plus(query)
        await page.goto(f"https://www.google.com/search?q={encoded_query}&num={limit*2}", wait_until='domcontentloaded', timeout=30000)

        # Handle Consent if needed (simple check)
        try:
        ...
        # Wait for results
        try:
            await page.wait_for_selector('.g', state='attached', timeout=5000)
        except:
            pass
        ...
        results = await page.evaluate('''() => {
            ...
            document.querySelectorAll('.g').forEach(el => {
                ...
                if (titleEl && linkEl) {
                    ...
                    const snipEl = el.querySelector('.VwiC3b') || el.querySelector('.IsZvec') || el.querySelector('.aCOpRe');
                    if (snipEl) snippet = snipEl.innerText;
