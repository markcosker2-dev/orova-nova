...
> [!important] OROVA: the lead source is SETTLED — do not add another one
> Nova's source is **contractor licence boards**: WA L&I (`m8qx-ubtq`), joined
> on `ubi` to principals (`4xk5-x9j6`) and insurance (`ciwg-agsx`). Free,
> keyless, and already yielding **8,768 qualified contractors ≈ 3.4 years of
> outreach, currently unworked.** More supply is not the constraint.
>
> When this skill reaches `references/data-sources.md`, treat it as background
> reading, **not a shopping list.** These are closed with evidence and must not
> be re-litigated: Apollo (no free-tier API — verified twice), Meta Ad Library
> (`ad_type=ALL` is EU/UK-only, US remodelers invisible), NPPES, Yelp paid,
> OpenCorporates, CALICO, med spas (ADR-0015).
>
> **Where this skill genuinely helps:** the *qualification* half —
> `local-prospecting.md` and `demand-signals.md` — applied to rows OROVA
> already has. Ranking 8,768 known contractors better is worth real money;
> finding an 8,769th is not.
>
> Storage has exactly one gate: `lead_validator.validate_lead_for_storage` →
> `_lead_repo.save_lead`. Anything this skill produces goes through it — never
> a side channel.
