...
> [!warning] OROVA: SMS is NOT an approved channel — read before using this skill
> OROVA has **no SMS channel today**, and turning one on is an architecture
> decision, not a skill invocation. Write the ADR first.
>
> Why this is stricter than voice: TCPA requires **prior express written
> consent** for marketing SMS, and an established business relationship is
> explicitly *not* sufficient (see `references/compliance.md`). Statutory
> damages run **$500–$1,500 per message**. OROVA's consent gate was built for
> calls to business lines; it does not capture written SMS consent, and
> pointing an SMS sender at the existing lead table would send unconsented
> marketing texts to every contractor in it.
>
> Prerequisites before any SMS send, all of them:
> 1. An ADR in `vault/40-decisions/` opening the channel
> 2. A2P 10DLC brand + campaign registration
> 3. A consent-capture path that records the disclosure and a timestamp
> 4. STOP/HELP handling wired and tested
> 5. The consent gate extended — **never loosened** — to cover SMS
>
> Until all five exist, this skill is reference material for *planning* that
> work. It is not authorisation to send.
>
> Note: this skill is written for DTC/ecommerce lifecycle SMS (abandoned cart,
> post-purchase, win-back). OROVA sells B2B services to licensed contractors —
> most of its sequence templates do not transfer.
