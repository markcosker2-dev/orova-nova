...
function resolveManifestPath(): string {
  ...
  if (isPackaged) {
    return join(process.resourcesPath, 'hermesclaw-extensions.json');
  }
  const appPath = electronApp?.getAppPath?.() ?? __dirname;
  return join(appPath, 'hermesclaw-extensions.json');
}
...
export async function loadExtensionsFromManifest(): Promise<void> {
  ...
  if (existsSync(manifestPath)) {
    ...
    logger.debug('[extensions] No hermesclaw-extensions.json found, loading all builtin extensions');
  }
