...
private async initDeviceIdentity(): Promise<void> {
     ...
     try {
      ...
      const identityPath = path.join((electronApp?.getPath?.('userData') ?? process.cwd()), 'hermesclaw-device-identity.json');
  ...
  private async connect(port: number, _externalToken?: string): Promise<void> {
    const startupHandshakeTimeouts = this.startLock || this.status.state === 'starting'
      ? {
