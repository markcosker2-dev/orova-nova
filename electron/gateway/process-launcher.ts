...
(function () {
  ...
  if (globalThis.__hermesclawFetchPatched) return;
  globalThis.__hermesclawFetchPatched = true;

  globalThis.fetch = function hermesclawFetch(input, init) {
    var url =
    ...
    if (url.indexOf('openrouter.ai') !== -1) {
      ...
      delete flat['x-title'];
      delete flat['X-Title'];
      flat['HTTP-Referer'] = 'https://github.com/NextAgentX/HermesClaw';
      flat['X-Title'] = 'HermesClaw';
      init.headers = flat;
    }
  ...
  if (process.platform === 'win32') {
    try {
      ...
      if (!cp.__hermesclawPatched) {
        cp.__hermesclawPatched = true;
...
function ensureGatewayFetchPreload(): string {
  const dest = path.join(electronApp?.getPath?.('userData') ?? '', 'gateway-fetch-preload.cjs');
  try {
