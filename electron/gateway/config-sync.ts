 ...
 * extensions.  If a previous HermesClaw version copied one of these into
...
const HERMES_OPENCLAW_BRIDGE_PLUGIN_ID = 'hermesclaw-bridge';
const LEGACY_HERMES_OPENCLAW_BRIDGE_PLUGIN_ID = 'hermesclaw-hermes-bridge';
...
function buildBundledPluginSources(pluginDirName: string): string[] {
  ...
  return [
    join(appPath, 'build', 'openclaw-plugins', pluginDirName),
...
export async function syncGatewayConfigBeforeLaunch(
  ...
  try {
    await syncHermesBridgeConfigForLaunch(appSettings);
  } catch (err) {
    logger.warn('Failed to sync Hermes bridge config before launch:', err);
  }
  ...
  try {
    ...
    // but never fully saved through HermesClaw UI).
    try {
...
async function syncHermesBridgeConfigForLaunch(
  appSettings: Awaited<ReturnType<typeof getAllSettings>>,
): Promise<void> {
  const shouldEnableBridge = isHermesClawBothMode(appSettings.runtime.mode);
  ...
  await withConfigLock(async () => {
    ...
    if (!shouldEnableBridge) {
      ...
      for (const pluginId of MANAGED_HERMES_OPENCLAW_BRIDGE_PLUGIN_IDS) {
        removed = removePluginRegistration(config, pluginId) || removed;
      }
      if (removed) {
    ...
    const existingEntry = entries[HERMES_OPENCLAW_BRIDGE_PLUGIN_ID] as Record<string, unknown> | undefined;
    const enabledEntry = {
      ...existingEntry,
      enabled: true,
    };

    const nextAllow = allow.includes(HERMES_OPENCLAW_BRIDGE_PLUGIN_ID)
      ? allow
      : [...allow, HERMES_OPENCLAW_BRIDGE_PLUGIN_ID];
    const nextPlugins = {
      ...plugins,
      enabled: true,
      allow: nextAllow,
      entries: {
        ...entries,
        [HERMES_OPENCLAW_BRIDGE_PLUGIN_ID]: enabledEntry,
      },
    };

    const allowChanged = nextAllow.length !== allow.length;
    const pluginsWereMissing = !config.plugins;
    const pluginEnabledChanged = plugins.enabled !== true;
    const entryChanged = !existingEntry || existingEntry.enabled !== true;

    if (!pluginsWereMissing && !allowChanged && !pluginEnabledChanged && !entryChanged) {
      return;
    }
...
const gatewayArgs = ['gateway', '--port', String(port), '--token', appSettings.gatewayToken, '--allow-unconfigured'];
   ...
   const electronApp = require('electron').app;
  const isPackaged = electronApp?.isPackaged ?? false;
  const mode = isPackaged ? 'packaged' : 'dev';
