...
import { handleOrovaRoutes } from './routes/orova';
...
export function startHostApiServer(ctx: HostApiContext, port = getPort('HERMESCLAW_HOST_API')): Server {
  ...
  server.on('error', (error: NodeJS.ErrnoException) => {
    if (error.code === 'EACCES' || error.code === 'EADDRINUSE') {
      logger.error(
        ...
        `Set HERMESCLAW_PORT_HERMESCLAW_HOST_API env var to override the default port.`,
      );
    } else {
