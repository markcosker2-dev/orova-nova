...
/**
 * Minimal logger indirection so this module stays importable outside the
 * Electron main process (unit tests, future standalone runtimes). The main
 * process swaps in the real file-backed logger via setFirewallLogger().
 */
...
let logger: FirewallLogger = console;

export function setFirewallLogger(customLogger: FirewallLogger): void {
  logger = customLogger;
}
...
/**
 * Shared policy file also loaded by the Python firewall
 * (app/core/semantic_firewall.py). Single source of truth for limits and
 * high-impact/delegation tool lists — edit config/firewall-rules.json, not
 * the fallbacks below.
 */
interface SharedFirewallPolicy {
  maxToolCallsPerTask?: number;
  maxAgentDepth?: number;
  defaultTaskTimeoutMs?: number;
  highImpactTools?: string[];
...
function loadSharedFirewallPolicy(): SharedFirewallPolicy {
  try {
    const candidates = [join(process.cwd(), 'config', 'firewall-rules.json')];
    if (typeof __dirname !== 'undefined') {
      candidates.push(join(__dirname, '..', '..', '..', 'config', 'firewall-rules.json'));
    }
    for (const candidate of candidates) {
      if (existsSync(candidate)) {
        return JSON.parse(readFileSync(candidate, 'utf-8')) as SharedFirewallPolicy;
      }
    }
  } catch {
...
const sharedPolicy = loadSharedFirewallPolicy();

export const DEFAULT_FIREWALL_CONFIG: FirewallConfig = {
  maxToolCallsPerTask: sharedPolicy.maxToolCallsPerTask ?? 20,
  maxAgentDepth: sharedPolicy.maxAgentDepth ?? 3,
  defaultTaskTimeoutMs: sharedPolicy.defaultTaskTimeoutMs ?? 5 * 60 * 1000,
  highImpactTools: sharedPolicy.highImpactTools ?? [
  ...
  delegationTools: sharedPolicy.delegationTools ?? [
    'delegate_task',
    'spawn_agent',
    'create_sub_agent',
  ],
...
function validateParameter(value: unknown, property: ParameterProperty, path: string): string | null {
  ...
  if (actualType === 'array') {
    ...
    const maxItems = property.maxItems ?? property.maxLength;
    if (maxItems !== undefined && arr.length > maxItems) {
      return `Parameter '${path}': array length ${arr.length} > maximum ${maxItems}`;
    }
    if (property.items) {
      for (let index = 0; index < arr.length; index += 1) {
        const itemError = validateParameter(arr[index], property.items, `${path}[${index}]`);
...
function createCredentialScopeRule(): FirewallRule {
  return {
    ...
    async evaluate(context: ToolCallContext): Promise<FirewallRuleResult> {
      ...
      const requiredCredentials = inferRequiredCredentials(context.toolName);
...
function inferRequiredCredentials(toolName: string): string[] {
