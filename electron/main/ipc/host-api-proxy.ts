...
export function registerHostApiProxyHandlers(): void {
  const hostApiPort = getPort('HERMESCLAW_HOST_API');
  ...
  ipcMain.handle('hostapi:fetch', async (_, request: HostApiFetchRequest) => {
    try {
      ...
      const path = typeof request?.path === 'string' ? request.path : String(request?.path ?? '');
      const errorMessage = error instanceof Error ? error.message : String(error);
      return {
        ok: false,
        error: {
          message: `Host API proxy failed for ${path}: ${errorMessage}`,
        },
      };
    }
  });
