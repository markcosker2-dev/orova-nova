...
import { getOpenClawStatus, getOpenClawDir, getOpenClawConfigDir, getOpenClawSkillsDir, ensureDir, listWslDistros } from '../utils/paths';
...
import { buildRuntimeInstallState } from '../runtime/services/runtime-install-orchestrator';
import { type InstallChoice } from '../runtime/types';
import {
...
import { scheduleHermesClawSharedConfigSync } from './hermesclaw-shared-config-sync';
import {
          ...
          if (request.action === 'save') {
            ...
            try {
              ...
              try {
                ...
                scheduleHermesClawSharedConfigSync('incremental');
              } catch (err) {
...
function registerCronHandlers(gatewayManager: GatewayManager): void {
  ...
  // UI-created tasks have no delivery target — results go to the HermesClaw chat page.
  ...
  ipcMain.handle('cron:create', async (_, input: {
    ...
    try {
      const gatewayInput = {
        ...
        wakeMode: 'next-heartbeat',
        sessionTarget: 'isolated',
        // UI-created jobs deliver results via HermesClaw WebSocket chat events,
...
function registerUvHandlers(): void {
  ...
  ipcMain.handle('uv:install-all', async (_, payload?: { installChoice?: InstallChoice }) => {
    try {
      const installChoice: InstallChoice = payload?.installChoice ?? 'openclaw';
      ...
      await setSetting('runtime', nextState.runtime);
      await setSetting('bridge', nextState.bridge);

      return { success: true, installChoice };
    } catch (error) {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        console.log(`[hermesclaw-validate] validating provider type: ${providerType}`);
