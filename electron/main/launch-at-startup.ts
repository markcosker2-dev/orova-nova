...
const LINUX_AUTOSTART_FILE = join('.config', 'autostart', 'hermesclaw.desktop');
...
function getLinuxDesktopEntry(): string {
  return [
    ...
    'Version=1.0',
  'Name=HermesClaw',
  'Comment=HermesClaw - AI Assistant',
...
async function applyLinuxLaunchAtStartup(enabled: boolean): Promise<void> {
   const targetPath = join(app?.getPath?.('home') ?? process.env.HOME ?? '', LINUX_AUTOSTART_FILE);
  if (enabled) {
