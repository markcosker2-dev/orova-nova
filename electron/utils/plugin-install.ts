...
// Note: `require('electron')` is used here to handle vite dev mode where app may not be initialized at module load
...
function readPackageDependencies(pkgJsonPath: string): string[] {
  try {
    const raw = readFileSync(fsPath(pkgJsonPath), 'utf-8');
...
function findPackagePathInNodeModules(nodeModulesDir: string, packageName: string): string | null {
  const packagePath = join(nodeModulesDir, ...packageName.split('/'));
  if (!existsSync(fsPath(join(packagePath, 'package.json')))) {
    return null;
  }
  return packagePath;
}

function hasInstalledPluginDependencies(targetDir: string): boolean {
  const pluginPkgJsonPath = join(targetDir, 'package.json');
  const pluginDeps = readPackageDependencies(pluginPkgJsonPath);
  if (pluginDeps.length === 0) {
    return true;
  }

  const targetNodeModules = join(targetDir, 'node_modules');
  return pluginDeps.every((depName) =>
    existsSync(fsPath(join(targetNodeModules, ...depName.split('/'), 'package.json'))),
  );
}

function resolvePluginDependencyRoots(realPath: string, npmPkgPath: string, targetDir: string): string[] {
  const candidates = new Set<string>();

  const realPathNodeModules = findParentNodeModules(realPath);
  const resolvedViaPnpmVirtualStore = realPath !== npmPkgPath && realPath.includes(`${path.sep}.pnpm${path.sep}`);
  if (realPathNodeModules && resolvedViaPnpmVirtualStore) {
    candidates.add(realPathNodeModules);
  }
  ...
  const topLevelNodeModules = findParentNodeModules(npmPkgPath);
  if (!topLevelNodeModules) {
    return [...candidates];
  }

  const fallbackRoots = new Set<string>();
  for (const depName of pluginDeps) {
    const depTopLevelPath = findPackagePathInNodeModules(topLevelNodeModules, depName);
    if (!depTopLevelPath) continue;
    try {
      const depRealPath = realpathSync(fsPath(depTopLevelPath));
      const depNodeModules = findParentNodeModules(depRealPath);
      if (depNodeModules) {
        fallbackRoots.add(depNodeModules);
      }
    } catch {
      // Ignore individual dependency resolution failures; best-effort fallback.
    }
  }
...
export function copyPluginFromNodeModules(npmPkgPath: string, targetDir: string, npmName: string): void {
  let realPath = npmPkgPath;
  try {
    realPath = realpathSync(fsPath(npmPkgPath));
  } catch (error) {
    logger.warn(
      `[plugin] Failed to resolve real path for ${npmName}, falling back to the raw node_modules path`,
      {
        npmPkgPath,
  ...
  const rootVirtualNMs = resolvePluginDependencyRoots(realPath, npmPkgPath, targetDir);
  if (rootVirtualNMs.length === 0) {
  ...
  const queue: Array<{ nodeModulesDir: string; skipPkg: string }> = rootVirtualNMs.map((nodeModulesDir) => ({
    nodeModulesDir,
    skipPkg: npmName,
  }));
...
export function ensurePluginInstalled(
  ...
  const targetInstalled = existsSync(fsPath(targetManifest));
  const targetDepsComplete = targetInstalled ? hasInstalledPluginDependencies(targetDir) : false;
  const npmName = !getIsPackaged() ? PLUGIN_NPM_NAMES[pluginDirName] : undefined;
  const npmPkgPath = npmName ? join(process.cwd(), 'node_modules', ...npmName.split('/')) : null;
  const hasDevFallbackSource = Boolean(
    npmPkgPath && existsSync(fsPath(join(npmPkgPath, 'openclaw.plugin.json'))),
  );
  ...
  if (targetInstalled) {
    if (!targetDepsComplete) {
      logger.info(
        `[plugin] Repairing ${pluginLabel} plugin because runtime dependencies are missing`,
      );
    }

    if (!sourceDir && !hasDevFallbackSource && targetDepsComplete) {
      return { installed: true }; // no available source to compare, keep existing healthy install
    }

    if (sourceDir) {
    const installedVersion = readPluginVersion(targetPkgJson);
    const sourceVersion = readPluginVersion(join(sourceDir, 'package.json'));
      if (targetDepsComplete && (!sourceVersion || !installedVersion || sourceVersion === installedVersion)) {
  ...
  if (!getIsPackaged()) {
    if (npmName && npmPkgPath) {
      if (hasDevFallbackSource) {
        const installedVersion = targetInstalled && existsSync(fsPath(targetPkgJson)) ? readPluginVersion(targetPkgJson) : null;
        const sourceVersion = readPluginVersion(join(npmPkgPath, 'package.json'));
        const requiresRepair = targetInstalled && !targetDepsComplete;
        if (requiresRepair || (sourceVersion && (!installedVersion || sourceVersion !== installedVersion))) {
          logger.info(
            requiresRepair
              ? `[plugin] Repairing ${pluginLabel} plugin from dev/node_modules because runtime dependencies are missing`
        ...
        } else if (targetInstalled && targetDepsComplete) {
...
export function buildCandidateSources(pluginDirName: string): string[] {
  return getIsPackaged()
    ...
    : [
      join(electronApp?.getAppPath?.() ?? '', 'build', 'openclaw-plugins', pluginDirName),
