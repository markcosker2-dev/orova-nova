...
logDir = join((app?.getPath?.('userData') ?? process.cwd()), 'logs');
     ...
     logFilePath = join(logDir, `hermesclaw-${timestamp}.log`);

     // Write a separator for new session (sync is OK — happens once at startup)
     const sessionHeader = `\n${'='.repeat(80)}\n[${new Date().toISOString()}] === HermesClaw Session Start (v${app?.getVersion?.() ?? 'dev'}) ===\n${'='.repeat(80)}\n`;
