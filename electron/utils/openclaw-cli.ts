...
// Helper to safely access app properties during vite dev mode
function getElectronApp(): typeof import('electron').app | undefined {
   // eslint-disable-next-line @typescript-eslint/no-require-imports
...
export function generateCompletionCache(): void {
  ...
  const child = spawn(execPath, [entryPath, 'completion', '--write-state'], {
    env: {
      ...
      OPENCLAW_EMBEDDED_IN: 'HermesClaw',
    },
    stdio: 'ignore',
    detached: false,
    windowsHide: true,
