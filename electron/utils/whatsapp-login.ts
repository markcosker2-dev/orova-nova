...
// contains HermesClaw's own devDependencies — packages that are NOT deps of openclaw
...
function resolveOpenClawPackageJson(packageName: string): string {
    ...
    // 2. Fallback to project-level deps (works in dev mode for HermesClaw devDependencies)
    try {
    ...
    private async connectToWhatsApp(accountId: string): Promise<void> {
        ...
        try {
            ...
            this.socket = makeWASocket({
                ...
                // mobile: false,
                // browser: ['HermesClaw', 'Chrome', '1.0.0'],
            });
