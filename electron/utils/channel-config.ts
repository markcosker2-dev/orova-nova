...
// schema validation errors.  HermesClaw falls back to DEFAULT_ACCOUNT_ID
...
export function removePluginRegistration(currentConfig: OpenClawConfig, pluginId: string): boolean {
...
export async function readOpenClawConfig(): Promise<OpenClawConfig> {
    ...
    try {
        ...
        if (content.trim().length === 0) {
            logger.warn('OpenClaw config file is empty; treating it as a transient startup state', CONFIG_FILE);
            return {};
        }
...
export async function writeOpenClawConfig(config: OpenClawConfig): Promise<void> {
    ...
    try {
        ...
        const tempConfigFile = `${CONFIG_FILE}.tmp`;

        await writeFile(tempConfigFile, nextConfig, 'utf-8');
        await rm(CONFIG_FILE, { force: true });
        await writeFile(CONFIG_FILE, nextConfig, 'utf-8');
        await rm(tempConfigFile, { force: true });
    } catch (error) {
