...
# Sentinel Guardian Health Monitoring
from app.services.sentinel import start_sentinel, stop_sentinel



# Sequential Task Queue - prevents OOM crashes on Render Free Tier
class SequentialTaskQueue:
    """Ensures only one resource-intensive task runs at a time."""
    ...
    def can_run_task(self, task_name: str) -> bool:
        """Check if a task can run (no conflicting tasks running)."""
        with self.task_lock:
            # Define conflicting task groups
            ...
            task_groups = {
                "scraping": scraper_tasks,
                "ads": ads_tasks,
                "invoicing": invoicer_tasks
            }
            ...
            if not task_group:
                return True  # Non-conflicting task, can run

            # Check if any task from this group is already running
            running_in_group = self.running_tasks.intersection(task_groups[task_group])
            return len(running_in_group) == 0

    def start_task(self, task_name: str) -> bool:
        """Mark a task as started. Returns True if successfully started."""
        with self.task_lock:
            if not self.can_run_task(task_name):
                logger.warning(f"Task {task_name} blocked - conflicting task running")
                return False

            self.running_tasks.add(task_name)
            logger.info(f"Task {task_name} started. Running tasks: {self.running_tasks}")
            return True
    ...
    def end_task(self, task_name: str):
        ...
        with self.task_lock:
            ...
            logger.info(f"Task {task_name} completed. Running tasks: {self.running_tasks}")
...
from app.core.telegram_topics import setup_all_topics, send_to_topic
...
logger.info(f"🔍 DEBUG: VERTICAL_NAME={repr(VERTICAL_NAME)}")

try:
    vertical_config = load_vertical(VERTICAL_NAME)
    logger.info(f"✅ Loaded Vertical Config: {VERTICAL_NAME}")
except Exception as e:
    logger.error(f"❌ Failed to load vertical: {e}")
...
async def dashboard_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🏢 **OROVA Mission Control**\n\n"
        f"🔗 {url}\n\n"
        f"6 screens: Task Board • Content Pipeline • Calendar • Memory Bank • Team Structure • Digital Office"
    )


async def setup_topics_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Creates standard forum topics in the configured group chat"""
    await update.message.reply_text("🔧 Setting up forum topics...")
    
    success, message = await setup_all_topics()
    await update.message.reply_text(message)


def log_to_topic(topic: str, entry: str):
    """Route log entries to appropriate topics"""
    send_to_topic(topic, f"`{_get_ts()}` {entry}")
...
async def handle_call_decision(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if call_id.startswith("approve_"):
        ...
        if cid in pending_calls:
            ...
            await query.edit_message_text(f"⏳ **Initiating call to {data['name']}...**")
            
            retell_id = await execute_call(data['phone'], data['name'], data['script'])
            if retell_id:
                await query.edit_message_text(f"✅ **Call Connected!**\nID: `{retell_id}`\nScript: _{data['script']}_")
            else:
                await query.edit_message_text("❌ **Call failed to connect.** Check Retell logs.")
            del pending_calls[cid]
        else:
            await query.edit_message_text("⚠️ **Call expired or not found.**")
...
async def check_reminders_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🔍 Checking calendar for upcoming meetings...")
    ...
    keyboard = [
        [
            InlineKeyboardButton("✅ YES", callback_data=f"approve_{call_id}"),
            InlineKeyboardButton("❌ NO", callback_data=f"deny_{call_id}")
        ]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    message = (
        f"📅 **Upcoming Meeting Detected**\n"
        f"👤 **Prospect:** {prospect}\n"
        f"⏰ **Time:** {meeting_time}\n\n"
        f"🤖 **Proposed Script:**\n"
        f"\"{script}\"\n\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"📨 MESSAGE RECEIVED: '{user_msg}' from {chat_id}")
    
    # Auto-detect CEO chat ID from first message
    if not _CEO_CHAT_ID:
        _CEO_CHAT_ID = str(chat_id)
        logger.info(f"🔑 Auto-detected CEO chat ID: {_CEO_CHAT_ID}")
    ...
    quick_responses = {
        "hello": "👋 Nova here. What do you need?",
        "hi": "👋 Boss. Ready.",
        "hey": "👋 Nova. Go.",
        "howdy": "👋 Working.",
        "what's up": "⚡ At your service.",
        "sup": "⚡ Ready.",
        "good morning": "🌅 Morning, Boss. What's the mission?",
        "good evening": "🌆 Evening. Ready to work.",
        "good night": "🌙 Resting if you need me. Otherwise, go.",
    }
    user_msg_lower = user_msg.lower().strip()
    if user_msg_lower in quick_responses or user_msg_lower in ["hello", "hi", "hey", "howdy"]:
        await update.message.reply_text(quick_responses.get(user_msg_lower, "⚡ Here."))
        return
    ...
    if re.search(r"run\s+luxury\s+remodeling\s+in\s+miami", user_msg_lower):
        await update.message.reply_text("✅ Starting luxury remodeling pipeline in Miami.")
    ...
    if match:
        ...
        await update.message.reply_text(f"✅ Starting pipeline for '{niche}' in {location}.")
    ...
    if match:
        niche = match.group(1).strip()
        await update.message.reply_text(f"🔍 Initiating lead hunt for '{niche}'.")
        # await lead_hunt.start(niche)
        return

    # 4. "Send emails"
    if re.search(r"send\s+emails?", user_msg_lower):
        await update.message.reply_text("✉️ Running email drafter.")
    ...
    if re.search(r"^stop$|stop\s+(all|everything|jobs?)|kill\s+(all|everything|jobs?)", user_msg_lower):
        await update.message.reply_text("⛔ STOPPING ALL SYSTEMS. Kill switch activated.")
    ...
    status_msg = None
    async def telegram_status_callback(msg: str):
        nonlocal status_msg
        try:
            if status_msg is None:
                status_msg = await context.bot.send_message(chat_id=chat_id, text=msg)
            else:
                await status_msg.edit_text(msg)
        except Exception as se:
            logger.warning(f"Status callback error: {se}")
    ...
    try:
        ...
        response, updated_history = await router.route(user_msg, chat_id, clean_history, status_callback=telegram_status_callback)
        context.user_data['history'] = updated_history
        
        # Cleanup status message before sending final response
        if status_msg:
...
# ── Scheduler Config (must be above class definitions) ──
...
# ── Pending email drafts awaiting CEO approval ──
pending_emails = {}


def _prune_pending():
    """[FIX-1] Remove expired entries from pending_calls, pending_emails and pending_leads."""
    now = time.time()
    for store in (pending_calls, pending_emails, pending_leads):
        expired = [k for k, v in store.items()
                   if isinstance(v, dict) and now - v.get("_ts", now) > _PENDING_TTL_SECONDS]
        for k in expired:
            del store[k]
    # Cap dicts at 100 entries max as safety net
    for store in (pending_calls, pending_emails, pending_leads):
        while len(store) > 100:
...
class MissionControlHandler(BaseHTTPRequestHandler):
    ...
    def _check_api_key(self):
        """[FIX-5] Validate API key — NO default fallback."""
    ...
    def do_GET(self):
        ...
        if path == "/api/clients":
            ...
            if not self._check_api_key():
                return
            from app.skills.approval_workflow import get_all_pending_approvals
    ...
    def do_POST(self):
        ...
        if path == "/api/clients":
            ...
            if result.get("successfully_paused"):
                _telegram_notify(
                    ...
                    + "\n".join(f"  — {p['adset_name']}: {p['pause_reason']}" for p in result["paused_details"])
                )
            ...
            if not self._check_api_key():
                return
            _append_log(f"🎯 Manual lead hunt (Client {client_id})")
            ...
            try:
                ...
                except Exception as se: _append_log(f"⚠️ Sheet sync fail: {se}")
            ...
            if not self._check_api_key():
                return
            _append_log("📊 CEO Report requested from Mission Control")
            try:
                from app.skills.perf_dashboard import generate_weekly_report
                report = generate_weekly_report()
            ...
            if not self._check_api_key():
                return
            _append_log("🔄 One-click pipeline execution started")
            ...
            try:
                ...
                try:
                    ...
                    _append_log(f"⚠️ Email drafter trigger failed: {ej}")

                _append_log(f"✅ Pipeline completed: found {len(raw_leads)} total, {len(qualified_leads)} qualified leads")
                ...
                _append_log(f"❌ Pipeline failed: {e}")
                _increment_error()
                return self._json_response({"status": "error", "error": str(e)}, 500)
            ...
            _append_log(f"💬 Chat (Client {client_id}): {message[:40]}...")
            try:
            ...
            if draft_id in pending_emails:
                ...
                _append_log(f"📤 CEO APPROVED email to {draft['company']} from Dashboard")
                try:
                    ...
                    if result.get("status") == "sent":
                        _append_log(f"✅ Email sent to {draft['company']}")
            ...
            if draft_id in pending_emails:
                ...
                _append_log(f"🚫 CEO denied email to {company} from Dashboard")
            ...
            _append_log(f"🔄 Pipeline '{pipeline_name}' triggered from Mission Control")
        ...
        elif path.startswith("/api/approvals/approve/"):
            if not self._check_api_key():
                return
            item_id = path.replace("/api/approvals/approve/", "")
            from app.skills.approval_workflow import approve_item
            result = approve_item(item_id)
            status_code = 200 if result.get("status") == "ok" else result.get("code", 400)
            return self._json_response(result, status_code)
        
        elif path.startswith("/api/approvals/deny/"):
            if not self._check_api_key():
                return
            item_id = path.replace("/api/approvals/deny/", "")
            from app.skills.approval_workflow import deny_item
            result = deny_item(item_id)
            status_code = 200 if result.get("status") == "ok" else result.get("code", 400)
            return self._json_response(result, status_code)
...
<span>⚠️ KILL SWITCH ACTIVE — ALL AGENTS HALTED</span>
...
<div class="col">
<div class="card">
<div class="card-h"><span class="card-title">📊 PERFORMANCE</span><button class="btn btn-outline" onclick="loadMetrics()" title="Refresh">↻</button></div>
<div class="card-body" id="metrics">Loading...</div>
</div>
</div>
<div class="col">
<div class="card">
<div class="card-h"><span class="card-title">🤖 AGENTS & TASKS</span><span style="font-size:10px;color:var(--g4)" id="agentCount"></span></div>
<div class="card-body agents" id="agents">Loading...</div>
</div>
</div>
<div class="col">
<div class="card">
<div class="card-h"><span class="card-title">🧠 AI STATUS</span><button class="btn btn-outline" onclick="loadAI()" title="Refresh">↻</button></div>
...
<div class="col">
<div class="card">
<div class="card-h"><span class="card-title">📜 ACTIVITY LOG</span><button class="btn btn-outline" onclick="loadLogs()" title="Refresh">↻</button></div>
...
<div class="col">
<div class="card">
<div class="card-h"><span class="card-title">📋 PENDING APPROVALS</span><span style="font-size:10px;color:var(--g4)" id="pendingCount"></span></div>
...
<div class="col">
<div class="card">
<div class="card-h"><span class="card-title">⏰ SCHEDULER</span></div>
<div class="card-body" id="scheduler">Loading...</div>
</div>
</div>
<div class="col">
<div class="card">
<div class="card-h"><span class="card-title">🔌 GATEWAY</span><button class="btn btn-outline" onclick="loadGateway()" title="Refresh">↻</button></div>
...
<div class="col">
<div class="card">
<div class="card-h"><span class="card-title">🎯 CURRENT TASKS</span><button class="btn btn-outline" onclick="loadTasks()" title="Refresh">↻</button></div>
...
<div class="metric"><span class="metric-label">Fast Lane</span><span class="metric-value" style="color:var(--grn)">✓ Active (2 min)</span></div>
<div class="metric"><span class="metric-label">Slow Lane</span><span class="metric-value" style="color:var(--grn)">✓ Active (30 min)</span></div>
<div class="metric"><span class="metric-label">Email Drafter</span><span class="metric-value" style="color:var(--grn)">✓ Active (30 min)</span></div>
<div class="metric"><span class="metric-label">Reply Monitor</span><span class="metric-value" style="color:var(--grn)">✓ Active (30 min)</span></div>
`;
}else{
...
<div class="metric"><span class="metric-label">MiMo</span><span class="metric-value">${d.mimo_connected?'✓':'✗'}</span></div>
`;
...
document.getElementById('approvals').innerHTML=html||'<span style="color:var(--grn)">✓ No pending approvals</span>';
...
<div class="metric"><span class="metric-label">Telegram</span><span class="metric-value">${d.telegram?'✓':'✗'}</span></div>
<div class="metric"><span class="metric-label">Google Sheets</span><span class="metric-value">${d.sheets?'✓':'✗'}</span></div>
<div class="metric"><span class="metric-label">Retell</span><span class="metric-value">${d.retell?'✓':'✗'}</span></div>
<div class="metric"><span class="metric-label">AgentMail</span><span class="metric-value">${d.agentmail?'✓':'✗'}</span></div>
`;
...
class UnifiedWebHandler(MissionControlHandler):
    ...
    def do_GET(self):
        ...
        # Standard Mission Control GET routes (includes dashboard)
        super().do_GET()
...
def start_unified_server():
    ...
    logger.info(f"🌐 Unified Web Server (Health + Mission Control) on port {port}")
...
async def run_ceo_fast_lane():
    ...
    _append_log("⚡ Fast Lane: Checking approvals & pending calls...")
    ...
    try:
        ...
        for idx, row in enumerate(rows[1:], start=2):
            ...
            if status == "Ready for Call":
                ...
                _append_log(f"🚨 Approval needed for {company} (Row {idx})")

                send_telegram_with_buttons(
                    f"🚨 *Authorization Needed*\n\n*Target:* {company}\n*Intel:* {intel}\n\nWhat is your command, CEO?",
                    [
            ...
            # Execute approved calls
                ...
                _append_log(f"📞 Calling {company} ({phone})...")
                ...
                try:
                    ...
                    if result.get("success"):
                        ...
                        _append_log(f"✅ Call connected! ID: {call_id}")
                        send_telegram_report(f"📞 *Call Initiated*\n\nNow calling *{company}*.\nCall ID: `{call_id}`")
                    else:
                        sheet.update_cell(idx, 8, "Call Failed")
                        _append_log(f"❌ Call failed: {result.get('error')}")
                except Exception as e:
                    _append_log(f"❌ Call error: {str(e)}")

    except Exception as e:
        _append_log(f"⚡ Fast Lane Error: {str(e)}")
        _increment_error()

    _update_agent_status("CEO Reporter", "idle")

# ── SLOW LANE: Lead Hunting (Every 60 min) ────────────────────
async def run_lead_hunt_slow_lane():
    ...
    try:
        ...
        _append_log(f"✅ Hunter: {text_result[:100]}...")
        ...
        if qualified_leads:
            try:
                ...
                _append_log(f"📊 Saved {len(qualified_leads)} QUALIFIED leads to sheet.")
            except Exception as se:
                _append_log(f"⚠️ Slow Lane: Sheet save failed: {se}")
        else:
            _append_log("⚠️ No qualified leads this cycle — all filtered out")
        ...
        # Report to Mark via Telegram with lead approval keyboards
        send_telegram_report(
            f"☀️ *Autonomous Hunt Report*\n\n"
            f"Query: '{query}'\n\n{text_result}\n\n"
            f"Runs today: {daily_counter + 1}/{MAX_RUNS_PER_DAY}"
        )
        
        # Send individual lead notifications with approval buttons
        for lead in qualified_leads:
            lead_id = f"lead_{int(time.time())}_{lead.get('title', '').replace(' ', '_')[:20]}"
            
            # Store lead in pending_leads
            ...
            lead_message = (
                f"🎯 *New Qualified Lead Found*\n\n"
                f"🏢 **{lead.get('title', 'Unknown')}**\n"
                f"📊 *HAWK Score:* {lead.get('qualification_score', 0)}/100\n"
                f"📧 *Email:* {lead.get('email', 'N/A')}\n"
                f"📱 *Phone:* {lead.get('phone', 'N/A')}\n"
                f"🔗 {lead.get('url', '')}\n\n"
                f"💡 {lead.get('snippet', '')[:150]}..."
            )
            ...
            send_telegram_with_buttons(
                lead_message,
                [
                    {"text": "✅ Approve", "callback_data": f"lead_approve_{lead_id}"},
                    {"text": "❌ Discard", "callback_data": f"lead_discard_{lead_id}"},
                    {"text": "📊 Score", "callback_data": f"lead_score_{lead_id}"}
                ]
            )

        daily_counter += 1
    except Exception as e:
        _append_log(f"❌ Hunter Error: {str(e)}")
...
async def run_reply_monitor():
    ...
    _append_log("📬 Reply Monitor: Scanning & categorizing new messages...")
    ...
    try:
        ...
        if results.get("status") == "success":
            ...
            for msg in messages:
                ...
                # ── Skip Nova's own emails and system bounces ────────────
                ...
                # ── This is a GENUINE new reply ─────
                _seen_reply_ids.add(msg_id)
                new_leads += 1

                # ── MSI: DNC Check — immediate, zero tolerance ────────
                ...
                # ── MSI: Signal Protocol for HOT/WARM leads ──────────
                if category in ("HOT", "WARM"):
                    ...
                    if category == "HOT":
                        send_revenue_alert(
                            ...
                            elite_score=85,
                            status="Reply Received — High Intent Signal",
                            projected_value="TBD",
                            next_action="Initiating Autonomous Appointment Setting sequence.",
                        )
                    else:
                        # WARM leads — log but don't alert
                        _append_log(f"[WARM] Reply from {sender}: {snippet[:60]}...")

            if new_leads > 0:
                _append_log(f"✨ Found {new_leads} NEW categorized replies!")
                ...
                _append_log("📬 No new prospect replies.")
        ...
        _append_log(f"❌ Reply Monitor Error: {str(e)}")
        _increment_error()
...
async def _try_book_meeting(sender, subject, snippet):
    ...
    try:
        ...
        _append_log(f"📅 Meeting intent detected from {sender}! Using AI to book...")
        ...
        if meeting_data.get("book"):
            ...
            if result.get("success"):
                _append_log(f"📅 Meeting booked with {sender}!")
                ...
                try:
                    ...
                    if ceo_email and "@" in ceo_email:
                        ...
                        email_body = (
                            ...
                            f"👤 Prospect: {sender}\n"
                            f"📅 Date/Time: {meeting_data.get('date')}\n"
                            f"📋 Topic: {meeting_data.get('topic', 'Follow-up')}\n"
                            f"⏱ Duration: {meeting_data.get('duration', 30)} min\n\n"
                            f"Summary: {snippet}\n\n"
                            f"The calendar event has been created. Check Mission Control for more info."
                        )
                        send_outreach(
                            to=ceo_email,
                            subject=f"🚀 New Lead Booked: {sender.split('@')[0]}",
                            body=email_body
                        )
                        _append_log(f"📧 Notification email sent to {ceo_email}")
                except Exception as ne:
                    logger.error(f"Failed to send CEO notification email: {ne}")

                send_telegram_report(
                    f"📅 *Meeting Auto-Booked!*\n\n"
                    f"👤 *With:* {sender}\n"
                    f"📋 *Topic:* {meeting_data.get('topic', 'Follow-up')}\n"
                    f"🗓 *When:* {meeting_data['date']}\n"
                    f"⏱ *Duration:* {meeting_data.get('duration', 30)} min\n\n"
                    f"Added to your Google Calendar ✅",
                    force=True
                )
            else:
                _append_log(f"❌ Calendar booking failed: {result.get('error')}")
                send_telegram_report(
                    f"⚠️ *Meeting Booking Failed*\n\n"
...
# ── EMAIL DRAFTER + APPROVAL GATE (Every 30 min) ────────────
async def run_email_draft_job():
    ...
    _append_log("✉️ Email Drafter: Checking for qualified leads needing outreach...")
    ...
    try:
        ...
        for idx, row in enumerate(rows[1:], start=2):
            ...
            if email and status in ("New", "Ready for Email"):
                _append_log(f"✉️ Drafting PERSONALIZED email for {company} ({email})...")

                # Use AI to draft a PERSONALIZED email — no generic templates
                try:
                    prompt = (
                        f"Write a cold outreach email from OROVA (premium AI lead gen agency) "
                        f"to {contact_name} at {company} ({url}).\n\n"
                        f"RULES — DO NOT VIOLATE:\n"
                        f"1. Greeting: '[Name]—' ONLY. No 'Hi', 'Hello', 'Dear', 'Hope this finds you well'\n"
                    ...
                    if any(gp in draft_lower for gp in generic_phrases):
                        _append_log(f"⚠️ REJECTED generic draft for {company}. Regenerating...")
                    ...
                    send_telegram_with_buttons(
                        ...
                        [
                            {"text": "✅ Send", "callback_data": f"approve_email_{draft_id}"},
                            {"text": "❌ Discard", "callback_data": f"deny_email_{draft_id}"}
                        ]
                    )
                    ...
                    drafts_created += 1
                    _append_log(f"✉️ Personalized draft sent to CEO for approval: {company}")

                except Exception as e:
                    _append_log(f"❌ Draft error for {company}: {str(e)}")

            if drafts_created >= 3:  # Max 3 drafts per cycle
                break

        if drafts_created == 0:
            _append_log("✉️ No leads needing email outreach right now.")

    except Exception as e:
        _append_log(f"❌ Email Drafter Error: {str(e)}")

    _update_agent_status("Outreach Agent", "idle")

# ── Telegram Callback: Email Approval ──────────────────────────
async def handle_lead_approval(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles Approve/Discard/Score buttons for lead notifications."""
    query = update.callback_query
    await query.answer()
    
    data = query.data
    if data.startswith("lead_approve_"):
        lead_id = data.replace("lead_approve_", "")
        if lead_id in pending_leads:
            lead = pending_leads[lead_id]
            _append_log(f"✅ CEO APPROVED lead: {lead['business']}")
            
            await query.edit_message_text(
                f"✅ *Lead Approved*\n\n"
                f"🏢 *Company:* {lead['business']}\n"
                f"📧 *Email:* {lead.get('email', 'N/A')}\n\n"
                f"Lead moved to outreach queue."
            )
            
            # Update database status
            DatabaseManager.query("UPDATE leads SET status = 'approved' WHERE id = ?", (lead.get('db_id'),))
            del pending_leads[lead_id]
        else:
            await query.edit_message_text("⚠️ *Lead expired or not found.*")
            
    elif data.startswith("lead_discard_"):
        lead_id = data.replace("lead_discard_", "")
        if lead_id in pending_leads:
            lead = pending_leads[lead_id]
            _append_log(f"🚫 CEO DISCARDED lead: {lead['business']}")
            
            await query.edit_message_text(
                f"🚫 *Lead Discarded*\n\n"
                f"🏢 *Company:* {lead['business']}\n"
                f"Lead has been removed from queue."
            )
            
            # Update database status
            DatabaseManager.query("UPDATE leads SET status = 'discarded' WHERE id = ?", (lead.get('db_id'),))
            del pending_leads[lead_id]
        else:
            await query.edit_message_text("⚠️ *Lead expired or not found.*")


async def handle_lead_score(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Shows full HAWK score breakdown when Score button is clicked."""
    query = update.callback_query
    await query.answer()
    
    lead_id = query.data.replace("lead_score_", "")
    if lead_id in pending_leads:
        lead = pending_leads[lead_id]
        score_data = lead.get('score_breakdown', {})
        
        breakdown_msg = (
            f"📊 *HAWK Full Score Breakdown*\n\n"
            f"🏢 **{lead['business']}**\n"
            f"🎯 *Total Score:* {score_data.get('total', 0)}/100\n\n"
        )
        ...
        for cat, score in categories.items():
            ...
            breakdown_msg += f"**{cat}:** {score}/100 | `{bar}`\n"
        
        breakdown_msg += (
            f"\n💡 *Reasoning:*\n{score_data.get('reasoning', 'No reasoning provided.')}\n\n"
            f"🔗 {lead.get('url', '')}"
        )
        
        await query.edit_message_text(breakdown_msg, parse_mode="Markdown", disable_web_page_preview=True)
    else:
        await query.edit_message_text("⚠️ *Lead expired or not found.*")


async def handle_email_decision(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handles ✅/❌ buttons for email drafts."""
    query = update.callback_query
    await query.answer()
    ...
    if data.startswith("approve_email_"):
        ...
        if draft_id in pending_emails:
            draft = pending_emails[draft_id]
            _append_log(f"📤 CEO APPROVED email to {draft['company']}. Sending...")
            ...
            if result.get("status") == "success":
                ...
                _append_log(f"✅ Email sent to {draft['to']}")
                ...
                await query.edit_message_text(f"❌ *Send Failed:* {result.get('message')}")
                _append_log(f"❌ Email send failed: {result.get('message')}")

            del pending_emails[draft_id]
        else:
            await query.edit_message_text("⚠️ *Draft expired or not found.*")

    elif data.startswith("deny_email_"):
        draft_id = data.replace("deny_email_", "")
        if draft_id in pending_emails:
            company = pending_emails[draft_id]["company"]
            _append_log(f"🚫 CEO denied email to {company}")
            ...
            del pending_emails[draft_id]
        await query.edit_message_text("🚫 *Email discarded.*")
...
def run_scheduler_loop():
    ...
    _append_log("🤖 OROVA Autonomy Loop: Online")
    _append_log(f"⚡ Fast Lane: Every {APPROVAL_CHECK_MINUTES} min")
    _append_log(f"🕵️ Slow Lane: Every {HUNT_INTERVAL_MINUTES} min")
    _append_log(f"✉️ Email Drafter: Every {EMAIL_DRAFT_INTERVAL_MINUTES} min")
    _append_log(f"📬 Reply Monitor: Every {REPLY_CHECK_MINUTES} min")
    ...
    # ── Mission Pulse (08:00 AM ET and 20:00 PM ET) ──────────────
    ...
    # ── Daily Metrics Snapshot (for history charts) ───────────────
    ...
    # ── Nightly Memory Compaction (00:00 UTC) ─────────────────────
    def run_memory_compaction():
        _update_agent_status("Oracle", "active", "Running memory compaction")
        try:
            from app.core.memory import memory
            summary = memory.compact_memory()
            _append_log(f"🧠 Memory compaction complete: {len(summary)} bullet summary generated")
            
            # Send summary to CEO
            if summary:
                summary_msg = "🧠 **EXECUTIVE DAILY SUMMARY**\n\n" + "\n".join(f"- {bullet}" for bullet in summary)
                _telegram_notify(summary_msg, force=True)
                _append_notification("Memory Compacted", "Daily executive summary generated", "system")
                
        except Exception as e:
            logger.error(f"Memory compaction failed: {e}")
            _increment_error()
        finally:
            _update_agent_status("Oracle", "idle")

    schedule.every().day.at("00:00").do(run_memory_compaction)

    # ── Uptime Persistence (Self-Ping) ──────────────────────────
    def run_uptime_ping():
        ...
        try:
            ...
            logger.info(f"🛰️ Uptime Ping: {url}")
            requests.get(url, timeout=10)
        except Exception as e:
            logger.warning(f"Uptime ping failed: {e}")

    schedule.every(5).minutes.do(run_uptime_ping)  # [FIX-11] 10→5 min to prevent Render 15-min idle kill
    ...
    while True:
        try:
            ...
            # ── Autonomy Heartbeat (Every 5 min) ──
            if time.time() - last_heartbeat > 300:
                _append_log("💓 Autonomy Heartbeat: Scheduler loop is active.")
            ...
            logger.error(f"🛑 CRITICAL: Scheduler Loop Error: {e}")
            _append_log(f"⚠️ Scheduler encountered an error: {e}")
            _increment_error()
...
def start_mission_control_server():
    ...
    logger.info(f"🏢 Mission Control API + Dashboard on port 8080")
    server.serve_forever()

# ── Telegram Commands for Audit Capabilities ──
async def cipher_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🔍 Running Cipher competitive sweep...")
    try:
        from app.skills.cipher_agent import CipherAgent
        ...
        await update.message.reply_text(f"❌ Cipher failed: {e}")

async def metaads_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("📊 Fetching Meta Ads performance...")
    try:
        from app.skills.meta_ads_agent import MetaAdsAgent
        agent = MetaAdsAgent()
        p = agent.get_account_performance("last_7d")
        if not p or p.get("error"):
            await update.message.reply_text(f"❌ Meta API error: {p.get('error', 'Unknown')}")
            return
        await update.message.reply_text((
...
async def metapause_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🔍 Evaluating Meta ad sets ({mode})...")
    try:
        from app.skills.meta_ads_agent import MetaAdsAgent
...
async def available_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for s in status["preferred_slots"]:
        ...
        msg += f"  {s['name']}: {s['start']}–{s['end']} ({days})\n"
...
async def slots_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not slots:
        ...
        return
    msg = "📅 *Available Slots (next 7 days):*\n\n"
    for s in slots:
...
def _handle_shutdown(signum, frame):
    ...
    try:
        ...
        logger.info("✅ Database backed up on shutdown.")
    except Exception as e:
        logger.warning(f"Shutdown backup failed: {e}")
    logger.info("👋 Nova going offline. Goodbye.")
    sys.exit(0)
...
def main():
    ...
    logger.info("🛰️ Phase 1: Starting Unified Web Server...")
    threading.Thread(target=start_unified_server, daemon=True).start()
    threading.Thread(target=run_scheduler_loop, daemon=True).start()
    
    # Start Sentinel Guardian Health Monitoring
    logger.info("🛡️ Starting Sentinel Guardian Health Monitoring Service...")
    start_sentinel()
    
    # Phase 2: Brief startup buffer for web server to bind
    logger.info("⏳ Phase 2: Waiting 3s for web server to initialize...")
    time.sleep(3)
    
    # Phase 3: Connectivity Verification
    logger.info("📡 Phase 3: Probing Telegram API connectivity...")
    try:
        ip = socket.gethostbyname("api.telegram.org")
        logger.info(f"🌐 DNS resolved: api.telegram.org -> {ip}")
    except Exception as e:
        logger.warning(f"⚠️ DNS Probe Failed: {e}. Attempting bot start anyway...")

    token = os.environ.get("TELEGRAM_BOT_TOKEN")
    logger.info("🚀 Phase 4: Connecting Nova to Telegram...")
    ...
    for attempt in range(max_retries):
        try:
            ...
            application.add_handler(CallbackQueryHandler(handle_lead_approval, pattern=r"^(lead_approve_|lead_discard_)"))
            application.add_handler(CallbackQueryHandler(handle_lead_score, pattern=r"^lead_score_"))
            application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_message))

            logger.info(f"✨ Nova is Online! (Attempt {attempt+1}) Standing by, CEO. 🦾")
            ...
            if attempt < max_retries - 1:
                ...
                logger.error("🛑 CRITICAL: Max retries reached. Nova remains offline.")
                raise
