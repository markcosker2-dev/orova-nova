...
def send_outreach(to: str, subject: str, body: str, inbox_id: str = None) -> Dict[str, Any]:
    """
    Send an email. 
    AUDIT FIX: Uses InboxRotationManager to rotate through domains and protect deliverability.
    Falls back to AgentMail SDK if no specific rotation accounts are configured.
    """
    from app.core.email_inbox_rotation import InboxRotationManager
    rotator = InboxRotationManager()
    
    # 1. Selection: Try to get a rotated sender
    # If a specific inbox_id is provided by the AI, we respect it (manual override)
    # Otherwise, we use the rotator to pick the best domain for this turn
    sender_account = None
    if not inbox_id:
        sender_account = rotator.get_available_sender()
        if not sender_account:
            return {"status": "error", "message": "All sending domains at daily cap. No emails can be sent today."}
    
    try:
        # 2. Execution: Determine sending method (SMTP vs API)
        # If the account has a password, we use SMTP (yagmail) as per the production audit fix
        if sender_account and sender_account.get("pass"):
            yag = rotator.get_yag(sender_account)
            yag.send(to=to, subject=subject, contents=body)
            sender_display = sender_account.get("user")
            method = "smtp"
        else:
            # Fallback to AgentMail API (original Nova behavior)
            client = _get_client()
            if not client:
                return {"status": "error", "message": "AgentMail client not initialized."}
            
            sender = inbox_id or sender_account.get("user") if sender_account else _get_nova_inbox()
            if not sender:
            ...
            method = "agentmail_api"

        logger.info(f"[+] Email sent to {to} via {method} ({sender_display})")
        
        # 3. Tracking: Record the send for quota and history
        if sender_account:
            rotator.record_send(sender_account, to)
