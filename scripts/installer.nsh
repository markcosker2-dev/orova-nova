; HermesClaw Custom NSIS Installer/Uninstaller Script
;
...
!macro customCheckAppRunning
  ...
  DetailPrint "Extracting HermesClaw runtime files. This can take a few minutes on slower disks or while antivirus scanning is active."
  ...
  ${if} $R0 == 0
    ...
    # This covers HermesClaw.exe (multiple Electron processes), openclaw-gateway.exe,
  ...
  ; Even if HermesClaw.exe was not detected as running, orphan child processes
  ...
  ; PowerShell path-based kill may miss processes if the old HermesClaw was installed
  ...
  ; "Can't modify HermesClaw's files" retry loop -> "HermesClaw 无法关闭" dialog.
  ;
  ...
  ; files (antivirus, indexing) causing a blocking "HermesClaw 无法关闭" dialog.
...
; "HermesClaw 无法关闭" retry dialog when the old uninstaller fails.
;
...
!macro customInstall
  ...
  ; Wait 60s before starting deletion to avoid I/O contention with HermesClaw's
...
!macro customUnInstall
  ...
  MessageBox MB_YESNO|MB_ICONQUESTION \
    "Do you want to remove HermesClaw application data?$\r$\n$\r$\nThis will delete:$\r$\n  • AppData\Local\hermesclaw (local app data)$\r$\n  • AppData\Roaming\hermesclaw (roaming app data)$\r$\n$\r$\nYour .openclaw folder (configuration & skills) will be preserved.$\r$\nSelect 'No' to keep all data for future reinstallation." \
    /SD IDNO IDYES _cu_removeData IDNO _cu_skipRemove

  _cu_removeData:
    ; Kill any lingering HermesClaw processes (and their child process trees) to
    ...
    RMDir /r "$LOCALAPPDATA\hermesclaw"
    RMDir /r "$APPDATA\hermesclaw"

    ; --- Retry: if directories still exist (locked files), wait and try again ---

    ; Check AppData\Local\hermesclaw
    IfFileExists "$LOCALAPPDATA\hermesclaw\*.*" 0 _cu_localDone
      Sleep 3000
      RMDir /r "$LOCALAPPDATA\hermesclaw"
      IfFileExists "$LOCALAPPDATA\hermesclaw\*.*" 0 _cu_localDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$LOCALAPPDATA\hermesclaw"'
        Pop $0
        Pop $1
    _cu_localDone:

    ; Check AppData\Roaming\hermesclaw
    IfFileExists "$APPDATA\hermesclaw\*.*" 0 _cu_roamingDone
      Sleep 3000
      RMDir /r "$APPDATA\hermesclaw"
      IfFileExists "$APPDATA\hermesclaw\*.*" 0 _cu_roamingDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$APPDATA\hermesclaw"'
        Pop $0
        Pop $1
    _cu_roamingDone:
    ...
    StrCpy $R3 ""
    IfFileExists "$LOCALAPPDATA\hermesclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $LOCALAPPDATA\hermesclaw"
    IfFileExists "$APPDATA\hermesclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $APPDATA\hermesclaw"
  ...
  _cu_enumLoop:
    ...
    RMDir /r "$R3\AppData\Local\hermesclaw"
    RMDir /r "$R3\AppData\Roaming\hermesclaw"
