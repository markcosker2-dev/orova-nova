...
# Post-installation script for HermesClaw on Linux
...
# Create symbolic link for HermesClaw app binary
if [ -x /opt/HermesClaw/hermesclaw ]; then
    ln -sf /opt/HermesClaw/hermesclaw /usr/local/bin/hermesclaw 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/HermesClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/HermesClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/HermesClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/HermesClaw/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/hermesclaw'
...
echo "HermesClaw has been installed successfully."
