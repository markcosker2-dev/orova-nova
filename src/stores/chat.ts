...
import { hostApiFetch, sendHermesRuntimeChatCompletion } from '@/lib/host-api';
...
function getHermesAssistantMessage(response: HermesChatCompletionResponse): RawMessage | null {
  const message = response.choices?.[0]?.message;
  if (!message) return null;

  const rawMessage: RawMessage & { tool_calls?: unknown; toolCalls?: unknown } = {
  ...
  if (message.tool_calls !== undefined) rawMessage.tool_calls = message.tool_calls;
  if (message.toolCalls !== undefined) rawMessage.toolCalls = message.toolCalls;

  return normalizeStreamingMessage(rawMessage) as RawMessage;
}

function buildHermesConversationMessages(messages: RawMessage[]): HermesConversationMessage[] {
...
const IMAGE_CACHE_KEY = 'hermesclaw:image-cache';
...
function cloneRawMessages(messages: RawMessage[]): RawMessage[] {
  return messages.map((message) => ({
    ...message,
    ...(Array.isArray(message.content) ? { content: structuredClone(message.content) } : {}),
    ...(
...
function loadHermesChatSnapshot(): HermesChatSnapshot | null {
  try {
    const raw = localStorage.getItem(HERMES_CHAT_SNAPSHOT_KEY);
    if (!raw) return null;

    const parsed = JSON.parse(raw) as Partial<HermesChatSnapshot>;
    if (parsed.version !== 1) return null;
    if (typeof parsed.currentSessionKey !== 'string' || !parsed.currentSessionKey) return null;

    return {
      version: 1,
      sessions: Array.isArray(parsed.sessions) ? parsed.sessions.filter((session): session is ChatSession => (
        Boolean(session)
        && typeof session === 'object'
        && typeof (session as ChatSession).key === 'string'
        && Boolean((session as ChatSession).key)
      )) : [],
      currentSessionKey: parsed.currentSessionKey,
      currentAgentId: typeof parsed.currentAgentId === 'string' && parsed.currentAgentId
        ? parsed.currentAgentId
        : getAgentIdFromSessionKey(parsed.currentSessionKey),
      sessionLabels: parsed.sessionLabels && typeof parsed.sessionLabels === 'object'
        ? parsed.sessionLabels
        : {},
      sessionLastActivity: parsed.sessionLastActivity && typeof parsed.sessionLastActivity === 'object'
        ? parsed.sessionLastActivity
        : {},
      messagesBySession: parsed.messagesBySession && typeof parsed.messagesBySession === 'object'
        ? Object.fromEntries(
          Object.entries(parsed.messagesBySession)
            .filter(([sessionKey, messages]) => sessionKey && Array.isArray(messages))
            .map(([sessionKey, messages]) => [sessionKey, cloneRawMessages(messages as RawMessage[])]),
        )
        : {},
    };
  } catch {
...
function saveHermesChatSnapshot(snapshot: HermesChatSnapshot): void {
  try {
    localStorage.setItem(HERMES_CHAT_SNAPSHOT_KEY, JSON.stringify(snapshot));
  } catch {
    // ignore localStorage quota/unavailability errors
  }
}

function ensureHermesSessionList(sessions: ChatSession[], currentSessionKey: string): ChatSession[] {
...
function buildHermesChatSnapshotFromState(state: ChatState): HermesChatSnapshot {
  const existing = loadHermesChatSnapshot();
  const currentSessionKey = state.currentSessionKey || DEFAULT_SESSION_KEY;
  const sessions = ensureHermesSessionList(state.sessions, currentSessionKey);
  const allowedSessionKeys = new Set(sessions.map((session) => session.key));
  allowedSessionKeys.add(currentSessionKey);

  const messagesBySession = Object.fromEntries(
    Object.entries(existing?.messagesBySession ?? {})
      .filter(([sessionKey]) => allowedSessionKeys.has(sessionKey)),
  ) as Record<string, RawMessage[]>;

  messagesBySession[currentSessionKey] = cloneRawMessages(state.messages);

  return {
    version: 1,
    sessions,
    currentSessionKey,
    currentAgentId: state.currentAgentId || getAgentIdFromSessionKey(currentSessionKey),
...
function persistHermesChatSnapshot(state: ChatState): void {
  if (useSettingsStore.getState().runtime.mode !== 'hermes') return;
  saveHermesChatSnapshot(buildHermesChatSnapshotFromState(state));
}

const initialHermesSnapshot = useSettingsStore.getState().runtime.mode === 'hermes'
  ? loadHermesChatSnapshot()
  : null;
const initialHermesSessionKey = initialHermesSnapshot?.currentSessionKey ?? DEFAULT_SESSION_KEY;
const initialHermesSessions = ensureHermesSessionList(initialHermesSnapshot?.sessions ?? [], initialHermesSessionKey);
...
export const useChatStore = create<ChatState>((set, get) => ({
  messages: cloneRawMessages(initialHermesSnapshot?.messagesBySession[initialHermesSessionKey] ?? []),
  loading: false,
  error: null,
  ...
  sessions: initialHermesSessions,
  currentSessionKey: initialHermesSessionKey,
  currentAgentId: initialHermesSnapshot?.currentAgentId ?? getAgentIdFromSessionKey(initialHermesSessionKey),
  sessionLabels: { ...(initialHermesSnapshot?.sessionLabels ?? {}) },
  sessionLastActivity: { ...(initialHermesSnapshot?.sessionLastActivity ?? {}) },
  ...
  loadSessions: async () => {
    if (useSettingsStore.getState().runtime.mode === 'hermes') {
      const snapshot = loadHermesChatSnapshot();
      if (!snapshot) return;

      const nextSessionKey = snapshot.currentSessionKey || DEFAULT_SESSION_KEY;
      set({
        sessions: ensureHermesSessionList(snapshot.sessions, nextSessionKey),
        currentSessionKey: nextSessionKey,
        currentAgentId: snapshot.currentAgentId || getAgentIdFromSessionKey(nextSessionKey),
        sessionLabels: { ...snapshot.sessionLabels },
        sessionLastActivity: { ...snapshot.sessionLastActivity },
      });
      persistHermesChatSnapshot(get());
      return;
    }
  ...
  loadHistory: async (quiet = false) => {
    ...
    const loadPromise = (async () => {
      ...
      // displayName (e.g. the configured agent name "HermesClaw") instead.
      ...
      if (useSettingsStore.getState().runtime.mode === 'hermes') {
        const snapshot = loadHermesChatSnapshot();
        const snapshotMessages = cloneRawMessages(snapshot?.messagesBySession[currentSessionKey] ?? []);
        const applied = applyLoadedMessages(snapshotMessages, null);
  ...
  sendMessage: async (
    ...
    try {
      ...
      const runtimeMode = useSettingsStore.getState().runtime.mode;
      if (hasMedia) {
      ...
      const OROVA_AGENT_IDS = ['nova', 'atlas', 'pixel', 'quill', 'hawk', 'closer', 'sentinel', 'echo', 'oracle', 'viper'];
      const activeAgent = targetAgentId || get().currentAgentId;
      const isOrava = activeAgent && OROVA_AGENT_IDS.includes(activeAgent.toLowerCase());

      if (isOrava) {
        clearHistoryPoll();
        try {
          const chatRes = await hostApiFetch<{ status: string; response: string }>('/api/hermesclaw/chat', {
            method: 'POST',
            body: JSON.stringify({
              message: trimmed,
              agent_id: activeAgent.toLowerCase()
            })
          });
          ...
          if (chatRes.status === 'ok') {
            ...
            throw new Error(chatRes.response || 'Failed to get response from OROVA agent');
          }
        } catch (e) {
          clearHistoryPoll();
      ...
      } else if (runtimeMode === 'hermes') {
        clearHistoryPoll();
        const hermesResult = await sendHermesRuntimeChatCompletion<HermesChatCompletionResponse>({
          messages: buildHermesConversationMessages(get().messages),
        });
        const assistantMessage = getHermesAssistantMessage(hermesResult);
        if (!assistantMessage) {
          throw new Error('Hermes compatibility endpoint returned no assistant message');
        }
