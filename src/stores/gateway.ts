...
function hasMeaningfulGatewayStatusChange(current: GatewayStatus, next: GatewayStatus): boolean {
  return current.state !== next.state || current.gatewayReady !== next.gatewayReady;
}
    ...
    gatewayInitPromise = (async () => {
      try {
          ...
          gatewayReconcileTimer = setInterval(() => {
            ...
            ipc.invoke('gateway:status')
              .then((result: unknown) => {
                ...
                if (hasMeaningfulGatewayStatusChange(current, latest)) {
                  console.info(
        ...
        try {
          ...
          const current = get().status;
          if (hasMeaningfulGatewayStatusChange(current, refreshed)) {
