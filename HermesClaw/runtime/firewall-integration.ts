...
import { SemanticFirewall, getSemanticFirewall, ToolCallContext, FirewallDecision, FirewallConfig } from './semantic-firewall';
...
export class FirewallIntegration {
  ...
  private installRpcInterceptor(): void {
    ...
    const originalRpc = this.gatewayManager.rpc.bind(this.gatewayManager);
    
    // We can't directly override the method, but we can provide a wrapper
    // that the application should use instead
  ...
  async spawnSubAgent(params: {
    ...
    const subSession = this.createSession({
    ...
    // Evaluate the delegation through firewall
    await this.evaluateToolCall({
