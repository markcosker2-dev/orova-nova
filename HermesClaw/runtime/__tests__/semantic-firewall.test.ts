 ...
 * Run with: npx vitest run HermesClaw/runtime/__tests__/semantic-firewall.test.ts
 */
...
describe('SemanticFirewall', () => {
  ...
  describe('Credential Scope (JIT)', () => {
    it('should allow root agent full access', async () => {
      const context = createTestContext({
        isSubAgent: false,
        toolName: 'api_request',
    ...
    it('should allow sub-agent with required credentials', async () => {
      const context = createSubAgentContext({
    ...
    it('should allow wildcard credentials', async () => {
      const context = createSubAgentContext({
        grantedCredentials: ['*'],
        toolName: 'api_request',
        parameters: { url: 'https://api.example.com', method: 'GET' },
  ...
  describe('Data Leakage Prevention', () => {
    ...
    it('should allow non-sensitive content', async () => {
      const context = createTestContext({
        toolName: 'write',
        parameters: { 
          path: '/tmp/readme.md', 
  ...
  describe('Statistics', () => {
    it('should track statistics correctly', async () => {
      ...
      // Deny
      await firewall.evaluate(createTestContext({ toolName: 'shell', parameters: { command: 'rm -rf /' } }));
      // Review
      ...
      expect(stats.byDecision.deny).toBe(0); // shell is require_human_approval not deny
...
console.log('Semantic Firewall tests loaded');